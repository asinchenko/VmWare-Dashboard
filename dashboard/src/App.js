...
const App = () => {
	const {
		...
		setResourcesToCustomers,
		cpuTotalAmount,
		setCPUTotalAmount,
		ramTotalAmount,
		setRAMTotalAmount,
		storageTotalSSDAmount,
		setStoragaTotalSSDAmount,
		storageTotalFCAmount,
		setStorageTotalFCAmount,
		storageTotalNLAmount,
		setStorageTotalNLAmount,
		hardWareDevices,
		setHardWareDevices,
		clientList,
		setClientList,
						...
						<div>
							...
							<Routes>
								...
								<Route
									...
									element={user ? <Equipment /> : <Navigate to='/login' />}
								/>
								<Route
									path='/vms'
									element={user ? <VMs /> : <Navigate to='/login' />}
								/>
								<Route
									path='/customers'
								...
								<Route
									path='/collocation'
									element={user ? <Collocation /> : <Navigate to='/login' />}
								/>
								{/* Apps */}
								<Route
									path='/kanban'
...
async function segmentClients(value, clientResourcesArray, clientsContract) {
	...
	for (const client of clientsContract) {
		...
		if (
			tagsArray.some(tag => value.name.includes(tag)) &&
			client.client != 'Other'
		) {
...
function calculateResources(value, clientResourcesArray) {
	let customerVM = {}
	const objIndex = clientResourcesArray.findIndex(
		e => Object.keys(e)[0] === value.customer
	)
	...
	if (objIndex != -1) {
		clientResourcesArray[objIndex][value.customer] = {
			vm_amount:
				clientResourcesArray[objIndex][value.customer].vm_amount === undefined
					? 0
					: clientResourcesArray[objIndex][value.customer].vm_amount + 1,
			...
			storage: {
				ssd:
					storage.ssd +
					clientResourcesArray[objIndex][value.customer].storage.ssd,
				fc:
					storage.fc +
					clientResourcesArray[objIndex][value.customer].storage.fc,
				nl:
					storage.nl +
					clientResourcesArray[objIndex][value.customer].storage.nl,
			},
	...
	if (Object.keys(customerVM).length > 0) clientResourcesArray.push(customerVM)
}
...
function updateWithReservedValues(clientResourcesArray, clientsContract) {
	...
	for (let client of clientsContract) {
		if (client.type.toLowerCase() === 'reserv') {
			const objIndex = clientResourcesArray.findIndex(
				e => Object.keys(e)[0] === client.client
			)
			if (objIndex == -1) {
				const deStringifyContract = { cpu: 0, ram: 0, ssd: 0, fc: 0, nl: 0 }
				deStringifyContract.cpu = Number(client.contract.cpu)
