...
const vcenerFetchVMDetails = async (url, headers) => {
	try {
		...
		throw new Error('vcenerFetchVMDetails fail')
	}
}
...
const vcenterGetVMData = async (backendUrl, headers) => {
	try {
		...
		for (let item of vals) {
			...
			for (let key in item) {
				if (key === 'memory_size_MiB') {
					...
					try {
						let request = await vcenerFetchVMDetails(
							`https://${process.env.VCENTER}/rest/vcenter/vm/` + item[key],
							headers
						)
		...
		throw new Error(`Error to fetch VM data`)
	}
}
