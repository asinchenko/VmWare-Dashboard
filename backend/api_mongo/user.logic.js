...
export default class User {
	...
	static async getUser({ filters = null, page = 0, userPerPage = 20 } = {}) {
		let query
		if (filters) {
			if ('name' in filters) {
	...
	static async deleteUSER(userId) {
		try {
			const findUSER = await user.findOne({ _id: ObjectId(userId) })
			if (!findUSER) {
				throw Error('Couldn`t find a user you are trying to delete')
			}
			const deleteUSER = await user.deleteOne({ _id: ObjectId(userId) })
			return findUSER
		} catch (e) {
	...
	static async verifyEmailRequest(_id, email, uniqueString) {
		try {
			...
			const mailOptions = {
				...
				html: `<p>Verify link: <a href=${
					'http://' +
					currentUrl +
					':4000/api/user/verify/' +
					_id +
					'/' +
					uniqueString
				}>Click Here!</a></p>`,
			...
			throw new Error(
				`Couldn't sent an email, please verify the nodemail service or verifyEmailRequest function`
			)
		}
	}
